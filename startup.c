...
void (* const vector_table[])(void) = {
    (void (*)(void)) ((uint32_t)&__stack_start__),
...
// The C runtime expects an _exit() handler to occur when all is said and done.
// Since our code never exits, we are providing a simple infinite loop stub.
void
_exit (int status)
{
    UNUSED(status);
